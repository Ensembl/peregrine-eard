...
fn op_minmax_s<F>(f: F) -> Result<Box<dyn Fn(&mut GlobalContext,&[usize]) -> Result<Return,String>>,String>
        where F: Fn(&[f64]) -> f64 + 'static {
    Ok(Box::new(move |ctx,regs| {
        let out = if ctx.is_finite(regs[1])? {
            f(ctx.force_finite_number(regs[1])?)
        } else {
            f(&[ctx.force_infinite_number(regs[1])?])
        };
        ctx.set(regs[0],Value::Number(out))?;
        Ok(Return::Sync)
    }))
}

pub(crate) fn op_min_s(_gctx: &GlobalBuildContext) -> Result<Box<dyn Fn(&mut GlobalContext,&[usize]) -> Result<Return,String>>,String> {
    op_minmax_s(|v| v.iter().cloned().reduce(f64::min).unwrap_or(f64::NAN))
}

pub(crate) fn op_max_s(_gctx: &GlobalBuildContext) -> Result<Box<dyn Fn(&mut GlobalContext,&[usize]) -> Result<Return,String>>,String> {
    op_minmax_s(|v| v.iter().cloned().reduce(f64::max).unwrap_or(f64::NAN))
}
