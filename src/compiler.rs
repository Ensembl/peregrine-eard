...
pub struct EarpCompiler {
    ...
    block_macros: HashMap<String,Box<dyn Fn(&[PTCallArg],(&[String],usize),usize) -> Result<Vec<PTStatement>,String>>>,
    expression_macros: HashMap<String,Box<dyn Fn(&[PTCallArg],usize) -> Result<PTExpression,String>>>
}
...
impl EarpCompiler {
    ...
    pub fn add_block_macro<F>(&mut self, name: &str, macro_cb: F) -> Result<(),String>
            where F: Fn(&[PTCallArg],(&[String],usize),usize) -> Result<Vec<PTStatement>,String> + 'static {
    ...
    pub fn add_expression_macro<F>(&mut self, name: &str, macro_cb: F) -> Result<(),String>
            where F: Fn(&[PTCallArg],usize) -> Result<PTExpression,String> + 'static {
        self.check_macro_name_unused(name)?;
        self.expression_macros.insert(name.to_string(),Box::new(macro_cb));
        Ok(())
    }

    pub(crate) fn apply_block_macro(&self, name: &str, args: &[PTCallArg], pos: (&[String],usize), context: usize) -> Result<Vec<PTStatement>,String> {
        let cb = self.block_macros.get(name).ok_or_else(|| format!("No such block macro \"{:?}\"",name))?;
        cb(args,pos,context)
    }

    pub(crate) fn apply_expression_macro(&self, name: &str, args: &[PTCallArg], context: usize) -> Result<PTExpression,String> {
